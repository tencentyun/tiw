文档转码接入有两种方式：
 ...
 - 2. [主动轮询方式](/开发指南/文档转码/主动轮询方式.md)（适用于没有在互动白板控制台注册回调的场景）

此文档主要介绍 `注册回调方式` 发起文档转码的基本流程。

## 准备工作
- `存储桶配置  `（文档转码后的资源文件存储依赖对象存储COS，使用文档转码功能前，请先进行 [存储桶配置](/控制台指南/存储桶配置.md)）。
> ! 如果没有进行 `存储桶配置`，转码后的文件存储在`互动白板的公共存储桶`里，存储有效时间是7天。7天后，公共存储桶里的转码文件自动删除，url的文件就会失效。建议您进行[存储桶配置](/控制台指南/存储桶配置.md)，将转码文件存储到您自己的存储桶里。
...
> 说明：
> 1. 接口说明，请参考 [文档转码相关接口](https://cloud.tencent.com/document/product/1137/40057)。  回调事件，请参考 [文档转码回调事件](/开发指南/事件通知/文档转码事件.md)。 
...
```
### 4. 使用转码结果
客户端 将转码结果，设置到 `互动白板` ，即可实现文档的 上/下一步、上/下一页等功能。白板接口，请参考 [白板接口说明](/客户端 API/Web/TEduBoardController.md)
