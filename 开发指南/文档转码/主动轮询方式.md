## 文档转码 接入流程(主动轮询方式)
主动轮询方式，因为受轮询间隔时长限制（推荐间隔`2s`轮询一次转码进度），无法及时获取到转码结果，所以更建议使用 [注册回调方式](/开发指南/文档转码/接入流程.md)。

## 准备工作
- `存储桶配置  `（文档转码后的资源文件存储依赖对象存储 COS，使用文档转码功能前，请先进行 [存储桶配置](/控制台指南/存储桶配置.md)）
...
> 说明：
> 1. 接口说明，请参考 [文档转码相关接口](/服务端 API/API 概览.md)
...
> 说明：
> 1. 白板接口，请参考 [白板接口说明](/客户端 API/Web/TEduBoardController.md)
