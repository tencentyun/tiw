...
>4. 实时录制结果文件只保存3天，3天后将被删除，建议在使用实时录制功能前进行[存储桶配置](../控制台指南/存储桶配置.md)，或者在获取到录制结果后自行对录制文件进行转存。
...
>! 实时录制结果的视频文件存储依赖对象存储 COS，使用实时录制功能前，请先进行[存储桶配置](../控制台指南/存储桶配置.md)
...
实时录制后台需要加入课堂并对课堂的音视频和白板进行录制，因此需要您提供一个录制后台进房的`RecordUserId`以及`RecordUserSig`，生成`RecordUserId`和`RecordUserSig`请参考 [如何计算 UserSig](https://cloud.tencent.com/document/product/647/17275)。

为了将录制后台的`RecordUserId`与普通用户进行区分，我们约定`RecordUserId`必须如下：
```
tic_record_user_{roomid}_{随机数}
```
其中，`{roomid}`为您真实的房间号，假如课堂的音视频房间`100241`，一个合法的实时录制`RecordUserId`为`tic_record_user_100241_100`。同时您需要提供`tic_record_user_100241_100`对应的`RecordUserSig`签名。
>!
...
在需要进行录制时，例如老师学生都已经准备好开始上课，您可以使用**开始录制**接口开始实时录制，在请求接口时，需要上一步准备好的用于录制后台加入课堂的`RecordUserId`和`RecordUserSig`，当实时录制开始时，如果您配置了回调地址，您将收到事件为**开始录制**的回调请求通知。
