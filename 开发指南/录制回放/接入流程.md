...
在白板与TRTC实例初始化完成后，通过以上定义的 `Handler` 来触发定时发送对时信息，代码中的`trtcCloud`以及`board`分别为已经初始化完成的 TRTC 实例及白板控制实例，初始化操作可以参考 [白板 SDK 集成文档](/快速入门/集成 SDK/Android.md) 。
...
在白板与 TRTC 实例初始化完成后，启动一个定时任务来实现定时发送对时信息，其中的 `trtcCloud` 和 `board` 分别为已经初始化完成的 TRTC 实例及白板控制实例，初始化操作可以参考 [白板 SDK 集成文档](/快速入门/集成 SDK/iOS.md) 。
...
下边以 `C++` 代码为例，代码中的`trtcCloud`以及`board`分别为已经初始化完成的 TRTC 实例及白板控制实例，初始化操作可以参考 [白板 SDK 集成文档](/快速入门/集成 SDK/Windows.md) 。
...
最后在白板与 TRTC 实例初始化完成后，启动定时器定时发送对时信息，其中的 `trtcCloud` 和 `board` 分别为已经初始化完成的 TRTC 实例及白板控制实例，初始化操作可以参考 [白板 SDK 集成文档](/快速入门/集成 SDK/Windows.md) 。
...
在需要进行录制时，例如老师学生都已经准备好开始上课，您可以使用 [开始录制](/服务端 API/实时录制相关接口/开始实时录制.md) 接口开始录制，在请求接口时，需要使用到上一步准备好的`RecordUserId`和`RecordUserSig`，当录制开始时，如果您配置了回调地址，您将收到事件为 [录制开始](/开发指南/事件通知/实时录制事件.md) 的回调请求通知。
...
在上课过程中，如果您不希望录制中间一段内容（例如课间休息），并且想要将暂停前和暂停后的视频放在一个录制任务结果中，您可以使用[暂停录制](/服务端 API/实时录制相关接口/暂停实时录制.md) 接口和 [恢复录制](/服务端 API/实时录制相关接口/恢复实时录制.md) 接口实现。
...
在课堂结束或者需要停止录制的时候，您可以使用 [停止录制](/服务端 API/实时录制相关接口/停止实时录制.md) 接口通知录制服务停止当前录制，如果您配置了回调地址，录制视频处理完成后，您将收到事件为 [录制停止](/开发指南/事件通知/实时录制事件.md#lztz) 的回调请求通知。
...
+ **设置录制事件回调地址**
通过 [控制台](/控制台指南/回调配置.md) 或者 [设置回调地址](/服务端 API/实时录制相关接口/设置回调地址.md) 接口都可以完成设置录制事件回调地址。在录制任务结束后，录制服务会把录制结果回调到已设置好的地址，您可以在收到回调后根据业务需要对录制结果进行记录或者其他操作。

+ **通过 [查询录制任务](/服务端 API/实时录制相关接口/查询实时录制任务.md) 接口主动查询**
在录制过程中或者停止录制后，您都可以使用 [查询录制任务](/服务端 API/实时录制相关接口/查询实时录制任务.md) 接口来查询录制任务的具体信息。
...
您可以使用 [创建视频生成任务](/服务端 API/录制视频生成（内测中）相关接口/创建录制视频生成任务.md) 接口来通知录制服务进行生成视频，在请求的时候可以选择使用不一样的混流布局来生成不一样的混流视频。
...
在视频生成任务完成后，您如果设置了回调地址，录制服务会把视频生成结果回调到指定的地址。另外，您也可以使用 [查询视频生成任务](/服务端 API/录制视频生成（内测中）相关接口/查询录制视频生成任务.md) 接口来主动查询视频生成结果。
