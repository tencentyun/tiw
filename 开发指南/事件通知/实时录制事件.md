...
当录制服务准备完成，开始录制时通知 App 后台

EventData结构定义：

| 参数名称     | 类型    | 描述                                 |
| ------------ | ------- | ------------------------------------ |
| TaskId       | String  | 录制已开始的录制任务 TaskId          |
| RoomId       | Integer | 录制的房间 Id                     |
| RecordUserId | String  | 当前任务用于录制服务进房的用户 Id |
...
| 参数名称     | 类型    | 描述                                 |
| ------------ | ------- | ------------------------------------ |
| TaskId       | String  | 录制已停止的录制任务 TaskId           |
| RoomId       | Integer | 录制的房间 Id                     |
| RecordUserId | String  | 当前任务用于录制服务进房的用户 Id |
...
| Error.Code       | String                    | 如果录制发生错误会有该字段，具体错误码描述请参考接口文档 [查询录制任务](https://cloud.tencent.com/document/product/1137/40067)              |
| Error.Message    | String                    | 如果录制发生错误的具体错误描述                                                                                                                  |
...
录制任务暂停超过30分钟会自动停止，当暂停了20分钟时，会通知 App 后台处理该任务
...
录制暂停超过30分钟会自动停止，会通知 App 后台已经自动停止当前任务
