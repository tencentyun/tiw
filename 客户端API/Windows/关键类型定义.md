...
| timSync | bool | 是否使用腾讯云IMSDK进行实时数据同步  |
...
| experimental | const char * | 实验性参数集，字典类型JSON串  |
...
| url | const char * | 自定义鼠标指针样式的URL，格式限制参考文档：https://developer.mozilla.org/zh-CN/docs/Web/CSS/cursor/url  |
...
| isStaticPPT | bool | 指定对于PPT文件是否启用静态转码（转为静态图片），默认转为H5动画（转码耗时较长）  |
...
| backgroundUrl | const char * | 白板背景图或者背景H5页面的URL  |
