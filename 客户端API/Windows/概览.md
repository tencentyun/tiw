...
| GetTEduBoardRenderProcessHandler | 获取SDK内部的CefRenderProcessHandler  |
...
| AddBoard | 增加一页白板  |
| DeleteBoard | 删除一页白板  |
| PrevStep | 上一步 每个Step对应PPT的一个动画效果，若当前没有已展示的动画效果，则该接口调用会导致向前翻页  |
| NextStep | 下一步  |
| PrevBoard | 向前翻页  |
| NextBoard | 向后翻页  |
| GotoBoard | 跳转到指定白板页  |
...
| onTEBBackgroundH5StatusChanged | 设置白板背景H5状态改变回调  |
