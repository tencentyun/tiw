...
| TEDU_BOARD_TOOL_TYPE_SQUARE | int | 空心正方形  |
| TEDU_BOARD_TOOL_TYPE_SQUARE_SOLID | int | 实心正方形  |
| TEDU_BOARD_TOOL_TYPE_CIRCLE | int | 空心正圆形  |
| TEDU_BOARD_TOOL_TYPE_CIRCLE_SOLID | int | 实心正圆形  |
| TEDU_BOARD_TOOL_TYPE_BOARD_CUSTOM_GRAPH | int |  |
...
| TEDU_BOARD_IMAGE_STATUS_READ_ERROR | int | 本地图片加载错误  |
...
| TEDU_BOARD_VIDEO_STATUS_WAITING | int | 缓冲中  |
| TEDU_BOARD_VIDEO_STATUS_PLAYING | int | 缓冲结束  |
...
| selectBoxColor | TEduBoardColor | 选框颜色  |
| textSize | int | 文本粗细  |
| textStyle | int | 文本样式  |
...
| systemCursorEnable | boolean |  |
| enableScaleTool | boolean | 是否启用原生系统光标  |
| proxyServer | String | 是否启用白板缩放移动工具的缩放功能，当设置为false，切换到缩放移动工具时缩放功能不可用。默认值为true  |
| syncFps | int | 可选，代理服务器  |
| nickname | String | 可选，信令同步频率，范围[5,20]  |
| boardContentFitMode | int |  |
| isAutoHideRemoteCursor | boolean |  |
| showRemoteSelectBox | boolean |  |
| showLocalOperator | boolean |  |
| showRemoteOperator | boolean |  |
| elementOperationAuthority | OperationAuthority |  |
| mathGraphEnable | boolean |  |
| scaleRange | int[] |  |
| offlineWarningTimeout | int |  |
| autoHideRemoteCursorTimeout | int |  |
...
| 属性 | 类型 | 字段含义 |
| --- | --- | --- |
| path | String | 快照文件存储的本地路径，包括文件名和后缀，只支持 png，例如：aaa/bbb/ccc/xxx.png  |
